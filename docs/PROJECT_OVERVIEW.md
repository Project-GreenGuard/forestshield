...
## 18.0 Engineering addendum (March 2026)

Authoritative for **as-built** behavior; use for demos and grading alignment.

| Topic | As built |
|-------|-----------|
| **ML inference** | **`process_sensor_data`** → **`CLOUD_RUN_PREDICT_URL`** (POST JSON); rule-based fallback; persists **`riskLevel`**, **`spreadRateKmh`**. |
| **API reads** | **`api_handler`** returns persisted fields; **`sensor_enrichment`** backfills legacy DynamoDB rows. |
| **Risk map query** | DynamoDB scan uses **`Attr('timestamp')`** for the 24h window. |
